...
def _write_class_init(tlobject, kind, type_constructors, builder):
    ...
    if tlobject.real_args:
        ...
        args = [
            f"{a.name}: {a.type_hint()}{'=None' if a.is_flag or a.can_be_inferred else ''}"
            for a in tlobject.real_args
        ]
    ...
    for arg in tlobject.real_args:
        if arg.can_be_inferred:
            ...
            if arg.is_vector:
                ...
                code = f'[{code} for _ in range(len(self.id))]'
    ...
    if arg.is_vector:
        ...
        if not any(f.is_flag for f in tlobject.args):
            ...
            def fmt_flag(flag):
                ...
                return fmt.format(f'self.{flag.name}', 1 << flag.flag_index)
